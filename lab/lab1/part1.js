...
// **************can type in console bakedGoods.xxx to see the available keys such as 'type', price, etc.
...
var printMenu = function(foodList){
   var newlist= _.groupBy(
     foodList,'type'
   );
   _.each(newlist.Cake,function(food) {
        console.log(food.name + ' ... $' + food.price)
   });
   _.each(newlist.Bread,function(food){
     console.log(food.name + ' ... $' + food.price)
   });
}
